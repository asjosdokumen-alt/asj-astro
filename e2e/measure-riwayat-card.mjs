 ...
 *
 * Jalankan: BASE_URL=http://localhost:4321 node e2e/measure-riwayat-card.mjs
 *
 * Port: 4321 — the dev server this repo actually uses (`npm run dev`). The line
 * above said 4322 until 2026-09-18, while the code below defaulted to 4321, so
 * copying the comment started a probe against a port nothing listens on. 4322 was
 * the stray port of the two orphan verify-*.mjs probes (deleted the same day);
 * 4323 is deliberate and unrelated — the three bottomnav probes need a SECOND
 * server instance, since they measure occlusion between two live pages.
 */
