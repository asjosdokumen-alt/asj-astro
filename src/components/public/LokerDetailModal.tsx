...
function StepTimeline({ sections }: { sections: RSection[] }) {
  ...
  return (<>
    ...
    {(incItems.length > 0 || excItems.length > 0) && (<div class="u-grid-auto u-grid-auto--form gap-4 mb-6">{incItems.length > 0 && <div class="bg-emerald-900/15 border border-emerald-500/30 rounded-2xl p-4"><h5 class="text-emerald-400 font-black text-[11px] uppercase tracking-widest mb-3"><Icon name="check-circle" class="mr-1" />{t('ui.include')}</h5><div class="flex flex-wrap gap-2">{incItems.map((x, i) => <span key={i} class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-900/40 text-emerald-300 border border-emerald-500/40 rounded-full text-[11px] font-bold">{String(x)}</span>)}</div></div>}{excItems.length > 0 && <div class="bg-rose-900/15 border border-rose-500/30 rounded-2xl p-4"><h5 class="text-rose-400 font-black text-[11px] uppercase tracking-widest mb-3"><Icon name="times-circle" class="mr-1" />{t('ui.exclude')}</h5><div class="flex flex-wrap gap-2">{excItems.map((x, i) => <span key={i} class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-rose-900/40 text-rose-300 border border-rose-500/40 rounded-full text-[11px] font-bold">{String(x)}</span>)}</div></div>}</div>)}
    {(benItems.length > 0 || perItems.length > 0) && (<div class="u-grid-auto u-grid-auto--form gap-4 mb-6">{benItems.length > 0 && <div class="bg-amber-900/15 border border-amber-500/30 rounded-2xl p-4"><h4 class="text-xs font-black text-amber-400 uppercase tracking-widest mb-3"><Icon name="star" class="mr-1.5" />{t('ui.benefit')}</h4><ul class="space-y-2">{benItems.map((x, i) => <li key={i} class="flex items-start text-xs text-slate-300"><Icon name="circle-check" class="text-amber-500 mt-0.5 mr-2 text-[11px]" />{String(x)}</li>)}</ul></div>}{perItems.length > 0 && <div class="bg-slate-900/60 border border-slate-700 rounded-2xl p-4"><h4 class="text-xs font-black text-sky-400 uppercase tracking-widest mb-3"><Icon name="clipboard-check" class="mr-1.5" />{t('ui.requirements')}</h4><ul class="space-y-2">{perItems.map((x, i) => <li key={i} class="flex items-start text-xs text-slate-300"><Icon name="check" class="text-emerald-500 mt-0.5 mr-2 text-[11px]" />{String(x)}</li>)}</ul></div>}</div>)}
...
export default function LokerDetailModal({ job, onClose }: Props) {
  ...
  return (
    <>
    <div class="fixed inset-0 u-modal-shell bg-black/70 backdrop-blur-md z-[150] flex items-center justify-center p-4" ref={containerRef} onClick={onBackdropClick}>
          ...
          <div class="flex items-start gap-4 mb-6">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex flex-wrap items-center gap-2">
                <span class="text-sky-400 font-mono text-xs font-bold">{job.code}</span>
                {isUrgent ? <span class="px-2.5 py-1 bg-amber-500 text-white rounded-full text-[11px] font-black animate-pulse"><Icon name="bolt" class="mr-1" />{t('status.urgent')}</span>
                : isOpen ? <span class="px-2.5 py-1 bg-emerald-600 text-white rounded-full text-[11px] font-black"><Icon name="door-open" class="mr-1" />{t('status.open')}</span>
                : <span class="px-2.5 py-1 bg-red-700/80 text-white rounded-full text-[11px] font-black"><Icon name="door-closed" class="mr-1" />{t('status.close')}</span>}
                {job.kuota && job.kuota !== '-' && <span class="px-2 py-0.5 bg-slate-800 text-slate-300 border border-slate-600 rounded-full text-[11px] font-bold"><Icon name="users" class="mr-1" />{t('ui.quota')}: {job.kuota}</span>}
              </div>
              ...
              {job.kategori && <div class="text-[11px] text-slate-500 mt-2"><Icon name="tag" class="mr-1 text-sky-500/70" />{jobCategoryLabel(job.kategori)}</div>}
            </div>
          </div>
          {total && <div class="bg-gradient-to-r from-emerald-900/40 to-sky-900/30 border border-emerald-500/40 rounded-2xl p-5 mb-6 text-center"><p class="text-[11px] font-bold uppercase tracking-[4px] text-emerald-400 mb-1"><Icon name="wallet" class="mr-1" />{t('ui.detail_total_title')}</p><p class="text-4xl font-black text-white tracking-wide">{total}</p><p class="text-[11px] text-slate-400 mt-1">{t('ui.detail_total_sub')}</p></div>}
          <StepTimeline sections={parsed.sections} />
          {syaratList.length > 0 && <div class="bg-slate-900/60 border border-slate-700 rounded-2xl p-5 mb-6"><h4 class="text-xs font-black text-sky-400 uppercase tracking-widest mb-4"><Icon name="clipboard-check" class="mr-1.5" />{t('ui.detail_syarat')}</h4><ul class="u-grid-auto u-grid-auto--form gap-x-6 gap-y-2.5">{syaratList.map((s, i) => <li key={i} class="flex items-start text-xs text-slate-300"><Icon name="check" class="text-emerald-500 mt-0.5 mr-2 text-[11px]" />{s}</li>)}</ul></div>}
