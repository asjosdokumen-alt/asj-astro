...
describe('apiClient — server error message is surfaced, not paraphrased', () => {
  ...
  // KONTRAK: kode error server ikut terbawa di Error yang dilempar.
  //
  // Bukan kelengkapan belaka. `AI_UNAVAILABLE` datang sebagai 503 dengan `code`
  // di body, dan TIGA permukaan (AiCvForm, SiswaBaruForm, AdminAiCopilot)
  // menyalakan AiUnavailableBanner dari `code` itu. Selama klien hanya membawa
  // `message`, satu-satunya cara membaca kode adalah mempertahankan `fetch`
  // mentah — persis alasan situs-situs itu tidak pernah pindah ke klien ini.
  // Membawa kodenya adalah yang membuat konversi mungkin tanpa kehilangan
  // kontrak degradasi §6.5 baris 3.
  it('membawa `code`, `status` dan `retryAfter` server di Error yang dilempar', async () => {
    state = { isLoggedIn: false, sessionToken: '' };
    nonOk(
      JSON.stringify({
        success: false,
    ...
    try {
      await apiClient('cekDataPelamar', [{}], opts);
    } catch (e) {
      err = e as ApiError;
    }
    if (!err) throw new Error('apiClient tidak melempar pada respons 503');
    expect(err.code).toBe('AI_UNAVAILABLE');
    expect(err.status).toBe(503);
    expect(err.retryAfter).toBe(5);
    // Pesan server tetap utuh — penambahan ini tidak menggantikan perilaku lama.
    expect(err.message).toBe('Asisten AI sedang tidak tersedia. Coba lagi beberapa saat ya!');
  });
});
