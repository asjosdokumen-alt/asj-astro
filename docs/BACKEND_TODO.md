...
| **29** | ✅ **SELESAI 2026-09-13 — diukur dari sini, ternyata BUKAN item owner-side.** | Baris lama bilang "belum pernah diukur; tidak ada gate-nya" dan §8 menaruhnya di daftar owner. **Salah separuh:** tidak ada *gate*-nya (itu benar), tapi mengukurnya cukup `curl` dari mesin ini — tidak butuh dashboard. Metode: `curl -o /dev/null -w '%{time_starttransfer}'` ke `/.netlify/functions/<entri>`, lalu ulangi untuk memisahkan cold dari warm. **Hasil (produksi, 2026-09-13):** cold setelah idle ~45 s → `ping` 0.75 s · `health` 0.78 s · `public` 0.80 s · `get-app-data` 0.90 s; sentuhan pertama sesi → 1.31–1.81 s; **warm** (hit ke-2 dst.) → 0.49–0.86 s. **Semua jauh di bawah target < 3 s**; **terburuk yang teramati 1,81 s**, margin ~1,2 s. Catatannya: `time_starttransfer` = TTFB, jadi angka ini **termasuk** RTT ke `ap-southeast-1`, yang menurut Phase E menyumbang 99,7 % latency sistem. **Yang tetap tidak ada:** gate otomatisnya — kalau ingin, itu pekerjaan kode kecil, bukan blocker. |
...
10. **#28–#30** — owner-side: rotasi `SESSION_SECRET`, revoke token (dua `ghp_`, lihat #30), dan
    **#29 sudah selesai** (cold start diukur dari sini: terburuk 1,81 s vs target 3 s — bukan item owner).
