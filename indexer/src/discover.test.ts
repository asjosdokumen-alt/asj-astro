...
describe('discover', () => {
  ...
  it('counts match the measured profile (design §1, + CJS entries)', () => {
    ...
    // as `files.length` below and `fileCount` in build.test.ts; MEASURED with
    // the sanctioned tool: 275 + 102 + 21 + 75 + 6 + 20 = 499.
    expect(count('astro')).toBe(21);
    ...
    // `.sh`, which no bucket counts — so the net is -3, not -4. MEASURED with
    // the sanctioned tool: 275 + 101 + 21 + 72 + 6 + 20 = 495.
    expect(count('mjs')).toBe(72);
