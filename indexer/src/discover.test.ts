...
describe('discover', () => {
  ...
  it('counts match the measured profile (design §1, + CJS entries)', () => {
    ...
    // 276 -> 275 (2026-09-24, company-profile reframe): -1 ts —
    // `src/components/public/PrincessMascot.test.ts`, deleted with the princess
    // mascot when the owner removed her from the site entirely. MEASURED with
    // the sanctioned tool after the file was gone: 275 ts + 102 tsx + 21 astro +
    // 75 mjs + 6 cjs + 20 js = 499.
    // 275 -> 276 (working tree, uncommitted): +1 ts for
    // src/layouts/BaseLayout.sakura.test.ts (the sakura-theme layout suite).
    // 276 -> 277 (working tree, uncommitted, item 11): +1 ts for
    // netlify/functions/contexts/jobs/repository.test.ts -- the numbering-ratchet
    // suite added with the GJ/Magang job-code work. MEASURED: the discover count
    // read 277 while this assertion said 276, and the only ts file added since
    // the sakura bump is that repository.test.ts.
    // 278 -> 279 (2026-09-26): +1 ts — src/store/adminTasks.ts, the "Papan Tugas
    // Tim" board's state, moved out of AdminPanel because the board became a
    // sidebar tab and a tab unmounts (component state would have dropped every
    // task). MEASURED with count-indexed.test.ts after the file existed, not
    // derived.
    expect(count('ts')).toBe(279);
    ...
    // 102 -> 101 (2026-09-24, company-profile reframe): -1 tsx —
    // `src/components/ui/Mascot.tsx`, deleted once its last three consumers
    // (`404.astro`, `LoginModal.tsx`, `StepGuide.tsx`) were rewired to sprite
    // icons (see the count('ts') note above). MEASURED with the sanctioned tool:
    // 275 + 101 + 21 + 75 + 6 + 20 = 498.
    expect(count('tsx')).toBe(104); // 46 at design time; modal/component test suites added since; re-anchored to HEAD by the R11 pass, then +9 when the 2026-09-19/20 backlog landed, then +2 for the LevelCard slice, then +1 for the StepGuide slice (2026-09-20), then +3 for HistoryTimeline.tsx + its .test.tsx + ContactForm.tsx (2026-09-21), then +1 for ContactForm.test.tsx (2026-09-21, same day), then +1 for AdminPanel.test.tsx (the Papan Tugas Tim suite). The last step is +1 and NOT +2: a `.test.tsx` file is a tsx file, so it moves this bucket and fileCount together, and it is exactly the kind of file that looks like it belongs in a "tests" bucket that does not exist. MEASURED with `indexer/src/count-indexed.test.ts` after the file existed: 277 + 103 + 22 + 76 + 6 + 20 = 504. +1 (2026-09-25): `src/components/candidate/AsjDossierCard.tsx`, the profile's identity card. It is the only file this change added — the guards for it went into the EXISTING `CandidateDash.test.tsx` rather than a new suite file, which is why this is +1 and not +2. MEASURED with the sanctioned tool: 277 + 102 + 20 + 70 + 6 + 20 = 495.
    ...
    // 22 -> 21 (2026-09-24, company-profile reframe): -1 astro —
    // `src/components/public/PrincessMascot.astro`, deleted with the mascot
    // herself when the owner removed her from the site entirely. Same -1 astro
    // as `files.length` below and `fileCount` in build.test.ts.
    // 21 -> 20 (2026-09-25): `src/components/public/SiteNav.astro`, deleted with
    // the band below the hero on `/` (owner ruling). The DELTA is the check —
    // exactly ONE `.astro` file left the tree, and this number moved by exactly
    // one, so the deletion is attributed rather than absorbed.
    // MEASURED with the sanctioned tool (`indexer/src/count-indexed.test.ts`):
    // 277 ts + 101 tsx + 20 astro + 70 mjs + 6 cjs + 20 js = 494.
    expect(count('astro')).toBe(20);
    ...
    // 20 = 501.
    // 75 -> 72 (2026-09-24, company-profile reframe): -3 mjs — the mascot's
    // gate and its two evidence tools, deleted with the mascot itself:
    // `e2e/test-mascot-motion.mjs`, `e2e/measure-mascot.mjs`,
    // `e2e/shot-mascot.mjs`. The gate's mutation battery
    // (`e2e/test-mascot-motion.mutations.sh`) went in the same commit but is a
    // `.sh`, which no bucket counts — so the net is -3, not -4.
    // 72 -> 70 (2026-09-25): `e2e/test-site-nav.mjs` and
    // `e2e/measure-site-nav.mjs`, deleted with the section bar they guarded and
    // measured. TWO `.mjs` files left, and this number moved by exactly two —
    // the same delta as `fileCount` in build.test.ts and `files.length` below,
    // which is the cross-check. MEASURED with the sanctioned tool:
    // 277 ts + 101 tsx + 20 astro + 70 mjs + 6 cjs + 20 js = 494.
    expect(count('mjs')).toBe(70);
    ...
    // mascot's own two: `PrincessMascot.astro` (-1 astro) and
    // `PrincessMascot.test.ts` (-1 ts). Same two deltas as count('astro') and
    // count('ts') above, which is the cross-check that nothing else moved with
    // them. MEASURED with the sanctioned tool, NOT derived:
    // 275 + 102 + 21 + 75 + 6 + 20 = 499.
    // 499 -> 498 (2026-09-24, company-profile reframe): -1 file —
    // `src/components/ui/Mascot.tsx` (-1 tsx), deleted once its last three
    // consumers were rewired to sprite icons. Same single delta as count('tsx')
    // above, which is the cross-check that nothing else moved with it. The 20
    // Aa-chan renders under `public/mascot/` were deleted in the same commit but
    // are images, and no bucket counts images. MEASURED with the sanctioned
    // tool, NOT derived: 275 + 101 + 21 + 75 + 6 + 20 = 498.
    // 498 -> 495 (2026-09-24, company-profile reframe): -3 files — the mascot's
    // own gate and its two evidence tools: `e2e/test-mascot-motion.mjs`,
    // `e2e/measure-mascot.mjs`, `e2e/shot-mascot.mjs`. Same -3 as count('mjs')
    // above, which is the cross-check that nothing else moved with them; the
    // battery (`e2e/test-mascot-motion.mutations.sh`) deleted alongside them is
    // a `.sh` and is outside every bucket. MEASURED with the sanctioned tool,
    // NOT derived: 275 + 101 + 21 + 72 + 6 + 20 = 495.
    expect(files.length).toBe(499); // 248 at design time; +4 Phase B kernel files, +5 CI gates/loader, +1 battery runner, +4 edge-validation gate, +2 e2e guards, +1 TabTambah test, +1 InputManualModal test, +1 settings-limit test, +1 404 page, -2 orphan e2e, +1 fetchMail test, +1 workflow gate, +1 CekSiswaModal test, +2 progress, +6 e2e probes, +1 candidates.test.ts (the phantom HEAD had been missing), +1 with-timeout watchdog, +1 ingestion xlsx-read test, +2 wire-payload contract tests, +1 fetch-boundary gate, -2 orphan e2e probes, +2 aiCvPairs registry + suite, +2 cvRows rules + suite, +2 cvPeriod rules + suite, +1 memory-archive gate, +2 publicSections module + suite, +3 public section primitives, +1 closing CTA band, +2 companyProfile + accentClass, +4 list primitives as tsx (-3 astro primitives they replaced), +1 desktop section nav, +1 team grid, +1 JobMiniList.tsx, +1 JobMiniList.test.ts, +3 evidence tools (shot-landing, measure-mini-rows, measure-hero), +1 ts measured-not-traced, +4 hero evidence tools, +2 site-nav gate + evidence tool, +2 gallery module + suite, +1 GalleryGrid.tsx, +1 measure-galeri.mjs, +1 Mascot.tsx, +2 mascot evidence tools (measure-mascot, shot-mascot), +1 Button test (2026-09-20), then +54 when the 2026-09-19/20 backlog was committed, then +2 for the LevelCard slice, then +3 for the JapanTexture band (2026-09-20), then +4 for the StepGuide slice + entrance animation (2026-09-20), then +5 for loker.astro, PersonGrid.tsx, LevelCard.tsx and LevelCard.test.tsx plus one unattributed file, then +1 for measure-theme-landing.mjs (2026-09-20) — the chain was re-anchored to HEAD by the R11 pass and is now one number again, then +1 for e2e/test-theme-gradients.mjs (2026-09-20, later the same day), then +2 for the mascot slice (2026-09-20): PrincessMascot.astro (+1 astro) and PrincessMascot.test.ts (+1 ts), then +1 for e2e/test-mascot-motion.mjs (2026-09-20), then +8 for the contact slice (2026-09-21): contexts/contact/{index,service,repository}.ts, netlify/functions/kontak.js, HistoryTimeline.tsx, HistoryTimeline.test.tsx, ContactForm.tsx, plus three earlier-session test files committed in the same commit. ATTRIBUTION corrected twice: the mascot slice was first written as +1 astro alone (it is TWO files — the component and its test), and the motion gate is a THIRD file whose battery is a `.sh` and therefore outside every bucket. Cross-checks: this is `fileCount` in build.test.ts and `count('mjs')` above, all three moved by the same `princess` + gate files. Re-measured at the END with count-indexed.test.ts (276 ts + 102 tsx + 22 astro + 75 mjs + 5 cjs + 20 js = 500), per R15 §3h. Then +1 (2026-09-21) for ContactForm.test.tsx — the missing suite for the contact island, whose absence was found by the render verification rather than by any gate. Then +3 (2026-09-22): count-indexed.test.ts (+1 ts), scripts/ci/verify-workflows.mjs (+1 mjs), scripts/ci/kf-mutate.cjs (+1 cjs). Attribution is by the per-bucket delta being exactly +1 in each of the three named buckets with `tsx`, `astro` and `js` unmoved. Re-measured again at the END with count-indexed.test.ts (277 ts + 102 tsx + 22 astro + 76 mjs + 6 cjs + 20 js = 503). -3 (2026-09-25): the band below the hero on `/` was deleted by owner ruling, taking `src/components/public/SiteNav.astro` (-1 astro) plus its e2e gate and evidence tool (-2 mjs). THIS number moved by exactly three, and `count('astro')` / `count('mjs')` above moved by exactly -1 and -2 — the two buckets that actually lost files and the total agreeing is what attributes the delta instead of absorbing it. Re-measured at the END with count-indexed.test.ts (277 ts + 101 tsx + 20 astro + 70 mjs + 6 cjs + 20 js = 494). +1 (2026-09-25): `src/components/candidate/AsjDossierCard.tsx`, the profile identity card. Same +1 as `count('tsx')` above and `fileCount` in build.test.ts, which is the cross-check. MEASURED at the END with count-indexed.test.ts (277 ts + 102 tsx + 20 astro + 70 mjs + 6 cjs + 20 js = 495). +1 (2026-09-25, later the same day): `src/lib/biodataExport.ts`, the shared "Download Full Biodata" formatter extracted out of the admin modal so the candidate dossier card and the admin panel produce the SAME file. A NEW `.ts` file moves this, `count('ts')` and `fileCount` together — the same +1 in all three, which is the cross-check. MEASURED at the END with count-indexed.test.ts (278 ts + 102 tsx + 20 astro + 70 mjs + 6 cjs + 20 js = 496). +3 (2026-09-26): the "Papan Tugas Tim" board and the Agenda card were moved out of the admin dashboard header into two sidebar tabs — `src/components/admin/TabTugas.tsx` (+1 tsx), `src/components/admin/TabAgenda.tsx` (+1 tsx) and `src/store/adminTasks.ts` (+1 ts, the board's state, which had to leave component state because a tab unmounts). Attribution is the per-bucket delta: `tsx` moved by exactly +2 and `ts` by exactly +1, with `astro`/`mjs`/`cjs`/`js` unmoved — and `count('ts')`, `count('tsx')` above and `fileCount` in build.test.ts all moved by the same amounts. MEASURED at the END with count-indexed.test.ts: 279 ts + 104 tsx + 20 astro + 70 mjs + 6 cjs + 20 js = 499.
  });
